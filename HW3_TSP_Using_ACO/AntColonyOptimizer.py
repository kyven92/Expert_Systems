...
class AntColonyOptimizer:
    ...
    def _evaporation(self):
        ...
        # self.heuristic_beta = self.heuristic_beta

    def _intensify(self, best_coords,best_score):
        """
        ...
        self.pheromone_matrix[i, j] += 1/best_score
    ...
    def fit(self, map_matrix, iterations=100, mode='min', early_stopping_count=20, verbose=True):
        ...
        for i in range(iterations):
            ...
            self._evaporation()
            self._intensify(best_path_coords,best_score)
    ...
    def plot(self):
        ...
        if not self.fitted:
            ...
            ax.text(.8, .6,
                    ...
                    bbox={'facecolor': 'gray', 'alpha': 0.8, 'pad': 10}, transform=ax.transAxes,weight='bold')
            ax.legend()
            plt.title("Ant Colony Optimization Results (best: {})".format(np.round(self.best, 2)))
            plt.savefig(f"ACO_Ants_{self.ants}_EVP_{self.evaporation_rate}_Alpha_{self.heuristic_alpha}_Beta_{self.heuristic_beta}.png")
            plt.show()
